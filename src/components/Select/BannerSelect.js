...
const CustomSelect = (props) => {
  ...
  useEffect(
    () => {
      console.log(data)
      handleResetData(data && data.bannerGroups && data.bannerGroups.length > 0
        ? data.bannerGroups
        : [])
    },
    [data, value]
  ...
  const handleResetData = (listBanners) => {
    let options = []
    if (listBanners && listBanners.length > 0) {
      options = listBanners.map((banner) => ({
        value: banner._id,
        label: banner.name,
        key: banner._id
      }))
  ...
  return (
    <Select
      ...
      placeholder={placeholder
        ? placeholder
        : <FormattedMessage
          id="Placeholder.SelectBanner"
          defaultMessage="Select Banner"
        />}
