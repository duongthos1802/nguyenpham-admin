...
export default {
  ...
  initQueryCreateOrUpdateCategory({ values, categoryId }) {
    ...
    if (values.typeBrand) {
      queryClause += `, typeBrand: ${values.typeBrand}`;
    } else {
      queryClause += `, typeBrand: null`;
    }
    ...
    queryClause += `, isHome: ${!!values.isHome}`;
    queryClause += `, isBrand: ${!!values.isBrand}`;
    queryClause += `, isMenu: ${!!values.isMenu}`;
    console.log("values.brands=====", values.brands);

    queryClause += `, brands: [`;
    if (values.brands && values.brands.length > 0) {
      values.brands.map((brand) => {
        queryClause += `, "${brand.value}"`;
      });
    }
    ...
    if (values.blogId) {
      queryClause += `, blogId: "${values.blogId.value}"`;
    } else {
      queryClause += `, blogId: null`;
    }
    ...
    if (values.metaKeyword) {
      const metaKeyword = stringHelper.removeEscapeCharacter(
        values.metaKeyword
      );
