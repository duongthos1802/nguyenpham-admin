...
// component
import { CustomForm, ErrorMessage, FooterForm } from "../../components";
...
const formikMap = withFormik({
  ...
  mapPropsToValues: ({ data }) => ({
    ...
    object: formikHelper.getDefaultValueField(data, "object", null),
...
const FormAction = (props) => {
  ...
  return (
    <CustomForm
      title={utils.initTitleForm(
        <FormattedMessage id="Page.FAQ" defaultMessage="FAQ" />,
        mode,
      )}
    >
      <Form {...FORM_ITEM_LAYOUT} onSubmit={handleSubmit} className={`row`}>
        <div className={`col-lg-8 col-12`}>
          ...
          <FormItem
            required={true}
            label={
              <FormattedMessage id="Label.Object" defaultMessage="Object" />
            }
            className="mb-0"
          >
            <FormattedMessage id="Label.Object" defaultMessage="Object">
              {(placeholder) => (
                <Input
                  ...
                  value={values.object}
                  onChange={(value) =>
                    setFieldValue("object", value.target.value)
                  }
                  onBlur={() => setFieldTouched("object", true)}
                  className={classNames({
                    "has-error": formikHelper.checkFieldError(
                      errors,
                      touched,
